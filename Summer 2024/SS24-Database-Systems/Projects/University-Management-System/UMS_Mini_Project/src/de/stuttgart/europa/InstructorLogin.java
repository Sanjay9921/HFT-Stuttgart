...
public class InstructorLogin extends JFrame implements ActionListener {
    ...
    public InstructorLogin(){
        ...
        this.setTitle("Instructor Login");
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == btnLogin){
            String insUserName = tfUserName.getText();
            String insPassword = new String(pfPassword.getPassword());
            String empID = "";

            System.out.println(insUserName + " " + insPassword);

            String query = "SELECT DISTINCT emp_id FROM INSTRUCTOR WHERE USER_ID = ? AND PSWD = ?";

            try{
                Conn c = new Conn();
                PreparedStatement preparedStatement = c.conn.prepareStatement(query);
                preparedStatement.setString(1, insUserName);
                preparedStatement.setString(2, insPassword);
                ...
                if(resultSet.next()){
                    ...
                    setVisible(false);
                    new instructor.InstructorMain(empID);
                }
                else{
